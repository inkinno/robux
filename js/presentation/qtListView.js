 ...
 * 세로/가로 반전 설계 (아코디언 구조), 독립 내부 스크롤, 수치형 최고기록 파티클/모션 및 최종완료 연출
 */
                        ...
                        <div class="quest-accordion-item ${isExpanded ? 'expanded' : ''} ${isQuestDone ? 'quest-done' : ''}" data-quest-id="${quest.id}">
                            <div class="quest-item-header">
                                <div class="quest-header-left">
                                    ...
                                    <span class="badge-type">${quest.natureType === 'fix' ? '고정형' : `자율형(${quest.stepGap}칸)`}</span>
                                ...
                                <div class="check-grid-scrollable">
                                    ${quest.checks.map((chk, cIdx) => `
                                        <button class="check-box-btn ${chk.done ? 'checked' : ''}" 
                                                ...
                                                style="margin-right: ${quest.natureType === 'custom' ? (quest.stepGap * 4) + 'px' : '0px'};">
                                            <span class="step-num">${chk.step}</span>
                                            ${chk.done ? `
    ...
    bindEvents() {
        ...
        headers.forEach(header => {
            header.addEventListener('click', (e) => {
                // 체크 버튼 클릭 시 아코디언 토글 방지
                if (e.target.closest('.check-box-btn')) return;
